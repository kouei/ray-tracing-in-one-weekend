...
__host__ __device__ vec3 reflect(const vec3 &v, const vec3 &n) { return v - 2 * dot(v, n) * n; }
...
__device__ vec3 random_in_unit_disk(curandState * local_rand_state) {
  while (true) {
    auto p = vec3(random_double(-1.0f, 1.0f, local_rand_state), random_double(-1.0f, 1.0f, local_rand_state), 0.0f);
